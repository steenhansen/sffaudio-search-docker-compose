...
function isMobileOne($http_user_agent)
{
...
function isMobileTwo($http_user_agent)
{
...
function leaveIfMobile($url_with_parameters)
{
    if (isMobileOne($_SERVER['HTTP_USER_AGENT'])) {
        redirectAfterHeader($url_with_parameters);
    } else if (isMobileTwo($_SERVER['HTTP_USER_AGENT'])) {
        redirectAfterHeader($url_with_parameters);
    } else {
        return;
    }
...
function getQueryParameters($widget_url, $get_author, $get_book, $get_view)
{
    if ($get_book) {
        if ($get_view) {

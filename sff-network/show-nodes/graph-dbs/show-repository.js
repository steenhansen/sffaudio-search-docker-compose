              ...
              UNION             
               ...
               UNION
                        ...
                        RETURN r_author_to_media, '' AS r_author_to_wiki, n_author, '' AS n_author_wiki, n_post AS n_book_or_post, v_db_version 
                          ORDER BY n_book_or_post.post_title
    ...
    class ShowRepository {
       ...
       static getAuthorNodes(strip_author, update_index) {
            var sql4 = ` // ShowRepository.getAuthorsNodes
                        MATCH (n_version:L_VERSION) 
			            RETURN  n_version.current_version+{update_index} AS v_db_version,
			            '' AS r_author_to_media, '' AS r_author_to_wiki, '' AS n_author, '' AS n_author_wiki, '' AS n_book_or_post
			                  
				UNION
                         MATCH (n_version:L_VERSION) 
                          WITH n_version.current_version+{update_index} as v_db_version, {strip_author} AS v_strip_author
                         MATCH (n_author:L_AUTHOR)-[r_author_to_media:L_AUTHOR_TO_BOOK]-(n_book:L_BOOK)
                         WHERE n_author.strip_author=v_strip_author
                           AND n_author.strip_author IN n_book.last_first_underscores
                           AND n_author.db_version=v_db_version
                        RETURN v_db_version, r_author_to_media, '' AS r_author_to_wiki, n_author, '' AS n_author_wiki, n_book AS n_book_or_post
              UNION             
                         MATCH (n_version:L_VERSION) 
                          WITH n_version.current_version+{update_index} as v_db_version, {strip_author} AS v_strip_author
                         MATCH (n_author:L_AUTHOR)-[r_author_to_wiki:L_AUTHOR_TO_WIKI]-(n_author_wiki:L_AUTHOR_WIKI)
                         WHERE n_author.strip_author=v_strip_author
                           AND n_author.db_version=v_db_version
                        RETURN v_db_version, '' AS r_author_to_media, r_author_to_wiki, n_author,  n_author_wiki, '' AS n_book_or_post
               UNION
                         MATCH (n_version:L_VERSION) 
                          WITH n_version.current_version+{update_index} as v_db_version, {strip_author} AS v_strip_author
                         MATCH (n_author:L_AUTHOR)-[r_author_to_media:L_AUTHOR_TO_POST]-(n_post:L_AUTHOR_POST)
                         WHERE n_author.strip_author=v_strip_author
                           AND n_post.strip_author=v_strip_author
                           AND n_post.db_version=v_db_version
                        RETURN v_db_version, r_author_to_media, '' AS r_author_to_wiki, n_author, '' AS n_author_wiki, n_post AS n_book_or_post 
                        ORDER BY n_book_or_post.post_title
...
MATCH (n_version:L_VERSION) 
    			...
    			UNION
                         MATCH (n_version:L_VERSION) 
                          WITH n_version.current_version as v_new_db_version, 'beyond-lies-the-wub' AS v_under_title
                        ...
                        ORDER BY n_pdf_or_rsd_or_podcast.pdf_title
                UNION                    
                         MATCH (n_version:L_VERSION) 
                          WITH n_version.current_version as v_new_db_version, 'beyond-lies-the-wub' AS v_under_title
                         ...
                         WHERE n_book.under_title = v_under_title 
                           AND n_book.db_version=v_new_db_version
                        RETURN r_author_to_book, r_book_to_media, '' AS r_book_wiki_to_book, n_author, n_book, '' AS n_book_wiki, n_pdf_or_rsd_or_podcast, v_new_db_version         
                        ORDER BY n_pdf_or_rsd_or_podcast.podcast_title    
                UNION 
                         MATCH (n_version:L_VERSION) 
                          WITH n_version.current_version as v_new_db_version, 'beyond-lies-the-wub' AS v_under_title
                         ...
                         WHERE n_book.under_title = v_under_title 
                           AND n_book.db_version=v_new_db_version
                        RETURN r_author_to_book, r_book_to_media, '' AS r_book_wiki_to_book, n_author, n_book, '' AS n_book_wiki,  n_pdf_or_rsd_or_podcast, v_new_db_version
                        ORDER BY n_pdf_or_rsd_or_podcast.rsd_title  
                UNION 
                         MATCH (n_version:L_VERSION) 
                          WITH n_version.current_version as v_new_db_version, 'beyond-lies-the-wub' AS v_under_title
                         ...
                         WHERE n_book.under_title = v_under_title 
                           AND n_book.db_version=v_new_db_version
                        RETURN r_author_to_book, r_book_to_media, '' AS r_book_wiki_to_book, n_author, n_book, '' AS n_book_wiki,  n_pdf_or_rsd_or_podcast, v_new_db_version 
                      ORDER BY n_pdf_or_rsd_or_podcast.post_title
				UNION	
						MATCH (n_version:L_VERSION) 
                         WITH n_version.current_version as v_new_db_version, 'beyond-lies-the-wub' AS v_under_title
 ...
 */
 ...
 static getBookNodes(under_title) {
			 ...
			 MATCH (n_version:L_VERSION) 
			            RETURN  n_version.current_version AS v_new_db_version, '' AS r_author_to_book, '' AS r_book_to_media, '' AS r_book_wiki_to_book, 
			                    '' AS n_author, '' AS n_book, '' AS n_book_wiki, '' AS n_pdf_or_rsd_or_podcast
    			UNION
                         MATCH (n_version:L_VERSION) 
                         ...
                         WHERE n_book.under_title = v_under_title  
                           AND n_book.db_version=v_new_db_version
                        RETURN v_new_db_version, r_author_to_book, r_book_to_media, '' AS r_book_wiki_to_book, n_author, n_book, '' AS n_book_wiki, n_pdf_or_rsd_or_podcast 
ORDER BY n_pdf_or_rsd_or_podcast.pdf_title
                UNION                    
                         MATCH (n_version:L_VERSION) 
                          WITH n_version.current_version as v_new_db_version, {under_title} AS v_under_title
...
ORDER BY n_pdf_or_rsd_or_podcast.post_title
				UNION	
                        ...
                        WHERE n_book.under_title = v_under_title 
                          AND n_book.db_version=v_new_db_version
						RETURN v_new_db_version, '' AS r_author_to_book, '' AS r_book_to_media, r_book_wiki_to_book, '' AS n_author, n_book, n_book_wiki,  '' AS n_pdf_or_rsd_or_podcast  
                     `;

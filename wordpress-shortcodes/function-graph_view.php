...
//https://www.sffaudio.com/?s=philip+k+dick => search_term=philip-k-dick
        ...
        //  kill leaveIfMobile
        // kill redirectAfterHeader
        static function new_mobileRedirect($mobile_leaving_pages, $heroku_address) // rename mobileHideGraph
        {
            if (is_page()) {
                if (function_exists('get_queried_object')) {
                    $php_slug = get_queried_object()->post_name;
...
if (!function_exists('graph_view_component')) {
    ...
    {  //  [graph_view_component]
        ...
        $db_version = SffGraphSearch::wakeUpSleepingGrapheneDb($widget_url . SffGraphSearch::WAKE_UP_GRAPHENE_DB);  // not needed, awake all the time
        ...
        <script>
            window.sff_php_vars={ 
                ...
                {    
                    window.sff_php_vars.js_is_desktop = false
                }
        </script>
        $web_html_javascript
JAVASCRIPT_HTML;
